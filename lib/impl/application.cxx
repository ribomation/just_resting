...
| |_| | |_| \__ \ |_|  _ <| |___ ___) || | | | | | | (_| |
 \___/ \__,_|___/\__|_| \_\_____|____/ |_| |_|_| |_|\__, |
    ...
    public:
        ...
        void set_any_port(bool any_port) {
            options.any_port = any_port;
        }
        ...
        void print(ostream& os) const {
            os << LOGO << name() << "/" << version() << "  [(C) Ribomation AB, 2018.]" << endl;
            if (options.debug) {
                os << "-- Routes --\n";
                dump_routes(os);
            }
    ...
    Application& Application::port(unsigned short port, bool any_port) {
        impl->set_port(port);
        impl->set_any_port(any_port);
        return *this;
    }
    ...
    void Application::launch(function<void(int)> onReady) {
        impl->launch(onReady);
    }
